...
function eventMousedown(task2idNum) {
   ...
   //show modal if handoff or collaboration is NOT being drawn
    if (DRAWING_HANDOFF != true && DRAWING_COLLAB != true){
       ...
       '<button class="btn " id="start-task" style="float :right " onclick="startTask('+task2idNum+')">Start</button>'+
       '<button class="btn " id="end-task" style="float :right " onclick="confirmCompleteTask('+task2idNum+')">Complete</button>' +
