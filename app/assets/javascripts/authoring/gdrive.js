...
var googleDriveLink = function(){
    ...
    gFolderBtn.onclick=function(){
        ...
        if((in_progress && flashTeamsJSON.folder) || (current_user == "Author" && flashTeamsJSON["startTime"])){
          logTeamActivity('Clicked Google Drive Project Folder', new Date().getTime(), current_user, chat_name, team_id, flashTeamsJSON); 
          window.open(flashTeamsJSON.folder[1]);
          

        }else{
          logTeamActivity('Clicked Google Drive Project Folder - Error Alert Triggered', new Date().getTime(), current_user, chat_name, team_id, flashTeamsJSON);
...
function createProjectFolder(){
  ...
  gapi.client.load('drive', 'v2', function() {
    ...
    logTeamActivity('Created Project and Task Folders', new Date().getTime(), current_user, chat_name, team_id, flashTeamsJSON);
    });
